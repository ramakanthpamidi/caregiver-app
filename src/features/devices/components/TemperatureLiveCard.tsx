...
import { evaluateTemperature, type HealthStatusLevel } from '../../../shared/lib/healthThresholds';
...
export default function TemperatureLiveCard({ tempC, ts }: Props) {
  const { lang } = useLanguage();
  const status    = tempC != null ? evaluateTemperature({ celsius: tempC }) : null;
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.headerRow}>
        <LinearGradient colors={[Colors.warningSoft, Colors.surface]} style={styles.iconBadge}>
          <Image
            source={require('../../../../assets/android-res/drawable/temperature.png')}
            style={styles.icon}
            resizeMode="contain"
          />
        </LinearGradient>
        <Text style={styles.title}>{t(lang, 'temperature')}</Text>
      ...
      {hasReading ? (
        <View style={styles.valueArea}>
          <View style={styles.valueRow}>
            ...
            <Text style={[styles.valueUnit, { color: theme.color }]}>°C</Text>
          </View>
          <Text style={styles.valueSecondary}>{cToF(tempC!)} °F</Text>
        </View>
      ) : (
        <View style={styles.emptyArea}>
          <Text style={styles.emptyText}>{t(lang, 'no_temp_reading')}</Text>
          <Text style={styles.emptyHint}>{t(lang, 'connect_thermometer')}</Text>
        </View>
      )}
      ...
      <View style={styles.footer}>
        <View style={styles.footerItem}>
          ...
          <Text style={styles.footerValue}>36.1 – 37.2 °C</Text>
        </View>
